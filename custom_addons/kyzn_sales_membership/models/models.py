...
class ProductTemplate(models.Model):
    ...
    durasi_hari = fields.Integer(string='Durasi Membership (Hari)')

class ResPartner(models.Model):
    _inherit = 'res.partner'

    is_member = fields.Boolean(string='Is a Member', default=False)
    join_date = fields.Date(string='Tanggal Join Pertama Kali', tracking=True)
    ...
    def _compute_age(self):
        for partner in self:
            if partner.birthdate:
                delta = relativedelta(date.today(), partner.birthdate)
...
class SaleOrder(models.Model):
    ...
    metode_pembayaran = fields.Selection([
        ...
        ('cc', 'Credit Card'),
        ('cash', 'Cash')
    ], string='Metode Pembayaran / Transaksi', tracking=True)
    ...
    membership_type = fields.Selection([
        ('adult', 'Adult'),
        ('academy', 'Academy'),
    ], string='Tipe Membership', tracking=True)

    tanggal_mulai = fields.Date(
        string='Start Date Membership',
        required=True,
        default=fields.Date.context_today,
        tracking=True
    )
    ...
    @api.depends('tanggal_mulai', 'membership_product_id')
    def _compute_tanggal_expiry(self):
        for rec in self:
            rec.tanggal_expiry = False
            if rec.tanggal_mulai and rec.membership_product_id:
                durasi_hari = rec.membership_product_id.product_tmpl_id.durasi_hari or 0
                if durasi_hari > 0:
                    rec.tanggal_expiry = rec.tanggal_mulai + relativedelta(days=durasi_hari)
    ...
    def _compute_follow_up_dates(self):
        for rec in self:
            ...
            if rec.tanggal_mulai and rec.jenis_transaksi == 'baru':
                rec.follow_up_1_date = rec.tanggal_mulai + relativedelta(months=1)
                rec.follow_up_2_date = rec.tanggal_mulai + relativedelta(months=2)
                rec.follow_up_3_date = rec.tanggal_mulai + relativedelta(months=3)

    @api.onchange('membership_product_id')
    def _onchange_membership_product_id(self):
        for rec in self:
            if rec.membership_product_id:
                tmpl = rec.membership_product_id.product_tmpl_id
                rec.membership_type = tmpl.membership_type
                if not rec.nilai_pembayaran:
